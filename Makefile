...
dev: deploy-init deploy-css deploy-rnnoise-binary deploy-lib-jitsi-meet deploy-libflac deploy-olm
	$(WEBPACK_DEV_SERVER) --detect-circular-deps --cert ../../ssl/certificate.crt --key ../../ssl/certificate.key --mode development --host 0.0.0.0 --disableHostCheck
...
build: compile deploy
	mkdir -p dist/css && \
	cp -r *.js *.html resources/*.txt connection_optimization favicon.ico fonts images libs static sounds LICENSE lang scripts/* dist/ && \
	cp css/all.css dist/css
